...
class EditCompanyPageState extends State<EditCompanyPage> {
  ...
  void _save() async {
    ...
    if (response.statusCode == 200) {
      ...
      AlertSnackBar.show(
          context: context,
          text: "Ocorreu um erro ao editar a empresa.",
  ...
  void _fetchCompanyData() async {
    final response =
        await makeHttpRequest(context, '/api/auth/Company/${widget.id}');
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        _corporateNameController.text =
            data['socialInscription']?.toString() ?? '';
        ...
        _requesterNameController.text =
            data['applicantsName']?.toString() ?? '';

        _requesterPhoneController.text =
            formatPhone(data['applicantsPhone']?.toString() ?? '');

        _companyEmailController.text = data['email']?.toString() ?? '';

        _companyPhoneController.text =
            formatPhone(data['companyPhone']?.toString() ?? '');

        _logoController.text = data['imageUrl']?.toString() ?? '';

        _postalCodeController.text =
            formatCep(data['addres']?['cep']?.toString() ?? '');

        _cityController.text = data['addres']?['city']?.toString() ?? '';

        _publicSpaceController.text =
            data['addres']?['patio']?.toString() ?? '';

        _neighborhoodController.text =
            data['addres']?['neighborhood']?.toString() ?? '';
      ...
      AlertSnackBar.show(
        context: context,
        text: "Ocorreu um erro ao buscar dados da empresa.",
      );
    }
  }
